...
def get_networkx_explaination_graph(edge_mask, g, dataset, filter=False):
    ...
    if filter:
        ...
        filter_inds = [
            i
            for i, tf in enumerate(filter_inds_bool)
            if tf == False and i not in nodes_important_bonds
        ]
        nxg.remove_nodes_from(filter_inds)
        node_weights = np.array(
            [i for ind, i in enumerate(node_weights) if int(ind) not in filter_inds]
        )
        node_label_dict = {
            k: v for k, v in node_label_dict.items() if int(k) not in filter_inds
        }
        shift = 0.01
...
def get_labelled_importance(feat_mask, dataset):
    ...
    feature_imp = {
        key: {
            k: (v - mean_importance) / std_importance
            for k, v in feature_imp[key].items()
        }
...
def get_top_features(feature_importance, n=10, filter_below_zero=False):
    ...
    for key in feature_importance.keys():
        ...
        top_features[key] = {
            k: v
            for k, v in sorted(
                feature_importance[key].items(), key=lambda item: item[1], reverse=True
            )[:n]
        }

    if filter_below_zero:
        top_features = {
            key: {k: v for k, v in top_features[key].items() if v > 0}
...
def get_feats_w_qtaim(feature_importance):
    ...
    for key in feature_importance.keys():
        qtaim_feats[key] = {
            k: v for k, v in feature_importance[key].items() if "extra_" in k
        }
    return qtaim_feats
...
def process_edge_mask(edge_mask_z, scale=True):
    ...
    a2b = torch.tensor(
        [
            float(torch.max(a2b[i], a2b[i + 1]))
            for i in range(int(list(a2b.shape)[0] / 2))
        ]
    )
    b2a = torch.tensor(
        [
            float(torch.max(b2a[i], b2a[i + 1]))
            for i in range(int(list(b2a.shape)[0] / 2))
        ]
    )
    ...
    if scale == True:
        ...
        edge_mask_processed = {"atom": atom, "bond": bond, "global": global_ft}
